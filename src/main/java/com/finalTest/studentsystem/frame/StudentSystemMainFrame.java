...
import com.finalTest.studentsystem.util.WindowUtil;
...
import static com.finalTest.studentsystem.util.Hitokoto.getHitokoto;
...
public class StudentSystemMainFrame extends JFrame {
	...
	private JMenu hitokoto ; 			//一言
	private JMenu about    ;           //关于
	...
	private JMenuItem author;	//作者信息
	private JMenuItem version;	//版本号
	private JMenuItem date;		//完成时间
	...
	public StudentSystemMainFrame(User user){
		...
		about = new JMenu("关于");
		author = new JMenuItem( "作者：魏子皓 杨泽鑫");
		version = new JMenuItem("版本号： V1.0");
		date = new JMenuItem(   "完成时间：2022/5/30");
		about.add(author);
		about.add(version);
		about.add(date);
		menuBar.add(about);

		hitokoto = new JMenu("每日一言："+getHitokoto());
		menuBar.add(hitokoto);
		...
		try {
			Image img = ImageIO.read(new File("src/main/java/1.png"));
